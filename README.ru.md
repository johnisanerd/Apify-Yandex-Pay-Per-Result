...
# 🔍 Yandex Search API: выдача в чистом JSON, оплата за страницу

> Одна фиксированная цена за страницу выдачи, все типы результатов включены. Удобный для разработчиков способ работать с Yandex Search API.
...
Этот Yandex Search API тарифицируется так: разовая плата за запуск поиска (Search Start) плюс фиксированная плата за каждую страницу выдачи, поэтому стоимость зависит от числа запрошенных страниц, а не от количества включённых типов результатов. Он выполняет запросы к Яндексу - ведущему поисковику в России и ряде рынков Восточной Европы и Центральной Азии - и возвращает чистый структурированный JSON, по одному элементу на каждый тип результата на странице. Каждый элемент содержит параметры поиска, счётчики результатов и массивы для органики, рекламы, графа знаний, изображений и видео. Поддерживаются региональные домены, таргетинг по языку и региону и пагинация.

> Изначально этот актор тарифицировался за результат, поэтому в его URL и в названии этого репозитория осталось "per-result", чтобы существующие ссылки продолжали работать. Сейчас оплата идёт за страницу, по тем же ценам, что и у нашего второго актора для Яндекса, [Scrape Yandex](https://apify.com/johnvc/Scrape-Yandex?fpr=9n7kx3).
...
**Одна цена за страницу.** Страница оплачивается один раз независимо от числа типов результатов, а страница без результатов по вашему выбору не оплачивается, что удобно для разведочных исследований, тестирования ключевых слов и задач, где плотность результатов меняется от запроса к запросу.
...
**Предсказуемая оплата по факту использования.** Тарификация за страницу плюс разовая плата за запуск, без подписки, а лимит `max_pages` держит расходы под контролем.
...
- **Типы результатов по выбору**: включайте органику, рекламу, граф знаний, изображения и видео по отдельности; включение дополнительных типов не повышает цену страницы
...
- **Оплата за страницу**: одна плата за запуск поиска, затем фиксированная цена за каждую доставленную страницу
