...
**Поддержка нескольких доменов.** Используйте `yandex.ru`, `yandex.com`, `yandex.com.tr` и другие региональные домены, сочетая их с `lang` и `lr` для точного контроля региона и языка.
...
- **Поиск в Яндексе** по всем основным региональным доменам
- **Контроль языка и региона** через `lang` и `lr`
- **Богатые типы результатов**: органика, реклама, граф знаний, изображения, видео
- **Контроль пагинации** через настраиваемый лимит `max_pages`
- **Оплата за результат**, привязанная к объёму собранных данных

### Качество данных
- **Один элемент на страницу** со стабильной структурой
...
| `yandex_domain` | `string` | Нет | `yandex.com` | Домен Яндекса, например `yandex.ru`, `yandex.com.tr`. |
...
| `max_pages` | `integer` | Нет | `2` | Максимум страниц; `0` = без лимита. Каждая страница - один платный элемент. |
| `output_file` | `string` | Нет | - | Имя файла для сохранения результатов. |

## Формат вывода

Реальный результат для `machine learning tutorial` (один элемент на страницу; массив `organic_results` сокращён до одного результата).
